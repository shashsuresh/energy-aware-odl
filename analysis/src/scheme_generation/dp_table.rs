...
use super::{searchable::Searchable, sparse_update::SparseUpdateSchemeGenerator};
...
impl DPSearch {
    ...
    /// One row for each value from 0 to last layer - `(available_options.len() / 4) + 1`
    /// One column for each value from 0 to `budget+1`
    pub fn new(budget: usize, available_options: Vec<UpdateSchemeCandidate>) -> Self {
        let solution_table = vec![vec![0.; budget + 1]; (available_options.len() / 4) + 1];
