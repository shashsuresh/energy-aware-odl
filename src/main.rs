...
use scheme_generation::{update_scheme_candidate, update_scheme_fitness, update_scheme_gen};
use std::io::Error;
use update_scheme_candidate::UpdateSchemeCandidate;
use update_scheme_fitness::UpdateSchemeFitness;
use update_scheme_gen::{GreedyMaxAccGenerator, SchemeGenerator};
...
fn main() -> Result<(), Error> {
    ...
    let update_cost = model.get_sparse_update_statistics((LAYER_IDX_TO_UPDATE.to_vec(),N_BIAS_TO_UPDATE,), LAYER_ITER_MAX);
